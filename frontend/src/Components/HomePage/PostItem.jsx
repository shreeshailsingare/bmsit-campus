...
import { Heart, MessageCircle, Bookmark, Share2, MoreHorizontal, ChevronLeft, ChevronRight, X } from "lucide-react";
import PostEditAction from "./PostEditAction";
import PostLikesComments from "./PostLikeCommentAction";
import TruncatedText from "./TruncatedText";
...
function PostItem({ post, onPostUpdate, onPostDelete, currentUser, setFlash }) {
  ...
  const video = post.media?.find(item => item.contentType?.startsWith("video/"));
  const images = post.media?.filter(item => item.contentType?.startsWith("image/")).slice(0, 4) || [];
  const allMedia = post.media?.filter(item => item.contentType?.startsWith("image/")) || [];
  ...
  return (
    <article className="feed-card">
      {/* Lightbox */}
      {selectedImg && (
        <div className="feed-lightbox" onClick={() => setSelectedImg(null)}>
          <button className="feed-lightbox-close" onClick={() => setSelectedImg(null)}>
            <X size={24} />
          </button>
          <img src={selectedImg} alt="Full view" onClick={(e) => e.stopPropagation()} />
        </div>
      ...
      <div className="feed-card-content">
        {/* Avatar */}
        <Link to={`/profile`} className="feed-avatar-link">
          <img
            src={post.author?.profile_image?.url || "https://cdn-icons-png.flaticon.com/512/847/847969.png"}
        ...
        <div className="feed-body">
          ...
          <div className="feed-header">
            <div className="feed-user-info">
              <Link to={`/profile`} className="feed-name">
                {post.author?.name}
              </Link>
              <span className="feed-username">@{post.author?.username}</span>
              <span className="feed-dot">·</span>
              <span className="feed-time">{formatTimeAgo(post.createdAt)}</span>
            </div>
          ...
          <div className="feed-text">
            <TruncatedText text={post.text} limit={280} />
          </div>
            ...
            <div className="feed-media">
              ...
              {video && (
                <div className="feed-video-wrapper">
                  <video src={video.url} controls className="feed-video" />
                </div>
              )}

              {/* Image Grid */}
              {images.length > 0 && (
                <div className={`feed-grid count-${images.length}`}>
                  {images.map((item, index) => (
                    <div
                      key={`img-${index}`}
                      ...
                      onClick={() => setSelectedImg(item.url)}
                    >
                      <img src={item.url} alt="post content" className="feed-grid-media" loading="lazy" />
                      {images.length > 1 && index === 0 && (
                        <span className="feed-grid-count">+{images.length}</span>
                      )}
                    </div>
                  ))}
                </div>
              ...
              {pdfs.map((item, index) => (
                <a
                  ...
                  className="feed-pdf"
                >
                  <div className="feed-pdf-icon">
                    <i className="fa-solid fa-file-pdf"></i>
                  </div>
                  <div className="feed-pdf-info">
                    <span className="feed-pdf-name">{item.filename || "Document.pdf"}</span>
                    <span className="feed-pdf-sub">PDF Document</span>
                  </div>
                  <i className="fa-solid fa-external-link-alt feed-pdf-ext"></i>
                </a>
              ))}
            </div>
          )}
